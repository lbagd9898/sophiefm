    ...
    <div className="group relative flex flex-col gap-3 h-full bg-white/70 backdrop-blur-sm border border-slate-200 rounded-2xl p-6 xl:p-8 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
      ...
      <div className="flex flex-col gap-1">
        <div className="flex items-start justify-between gap-2 flex-wrap">
          <h3 className="text-slate-900 font-semibold text-xl leading-snug group-hover:text-purple-800 transition-colors duration-200">
            {job.title}
          </h3>
          {job.current && (
            <span className="shrink-0 text-sm font-medium text-purple-700 bg-purple-100 px-2.5 py-0.5 rounded-full border border-purple-200">
              Current
            </span>
          )}
        </div>

        <p className="text-purple-700 font-medium text-base tracking-wide">
          {job.company}
        </p>

        {dateRange && (
          <p className="text-slate-400 text-sm font-light tracking-wider uppercase mt-0.5">
            {dateRange}
          </p>
        )}
      </div>
